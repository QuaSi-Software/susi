...
export default function getOutputRefs(
	...
	if (!outputRefs || typeof outputRefs !== 'object') {
		logError(`Node ${sourceNode.data.content} has no output_refs or connections.output_order`);
		return [];
	}

	/** Use the medium data to generate connection handles */
	const mediumNodeInputs = sourceNode.data.nodeInputs.filter((e) => e.type === NodeInputType.MEDIUM);
	...
	for (const [_, nodeID] of Object.entries(outputRefs)) {
		outputRefArray.push(nodeID);
	}
	if (!nodeData.import_data?.connection_handles) {
		for (const [mediumVarName, targetNodeName] of Object.entries(outputRefs)) {
			...
			connectionHandles[targetNodeName] = { source: handleIndex };
		}
